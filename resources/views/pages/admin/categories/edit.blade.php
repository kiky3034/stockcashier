<x-layouts.app :title="__('Edit Category')">
    <div class="space-y-6 p-4 sm:p-6 lg:p-8">
        <x-page-header
            title="Edit Category"
            description="Perbarui informasi category produk."
        >
            <x-slot:actions>
                ...
                <x-ui.card>
                    <div class="space-y-6">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-slate-500">Update nama dan deskripsi category.</p>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="description"
                                      name="description"
                                      rows="5"
                                      class="mt-2 block w-full rounded-2xl border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-800 shadow-sm transition focus:border-sky-400 focus:bg-white focus:ring-4 focus:ring-sky-100">{{ old('description', $category->description) }}</textarea>
                    ...
                    <x-ui.card>
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-slate-500">Ringkasan category saat ini.</p>
                            </div>

                            <div class="rounded-2xl bg-sky-50 p-4 ring-1 ring-sky-100">
                                <div class="font-semibold text-slate-900">{{ $category->name }}</div>
                                <div class="mt-1 font-mono text-xs text-sky-700">{{ $category->slug }}</div>
                            </div>
